//! The `wardn` CLI surface (see `WARDN_SPECIFICATION.md`'s "CLI Surface"
//! section). Lives in the library crate — not `main.rs` — so it can be
//! exercised directly by integration tests instead of only through a
//! spawned subprocess; `main.rs` is just `wardn::cli::run().await`.
...
use crate::{Db, config, members::Member, org::Org, roles::Role, serve};
...
fn init_tracing(serve_mode: bool) -> Result<()> {
    ...
    #[cfg(feature = "observability")]
    {
        ...
        let otel_layer = config::observability::otel_exporter_otlp_endpoint()
            .as_deref()
            .map(crate::telemetry::init_tracer)
            .transpose()?
        ...
        let loki_layer = match config::observability::loki_url() {
            Some(url) => {
                let (layer, task) = crate::telemetry::init_loki_layer(&url)?;
                loki_task = Some(task);
                Some(layer)
            }
            None => None,
...
pub async fn cmd_init(db_path: &str, name: Option<String>) -> Result<()> {
    let db = Db::open(db_path).await?;
    if let Some(existing) = crate::org::get(&db.conn).await? {
    ...
    let org = crate::org::create(&db.conn, name.trim()).await?;
    println!("Created database at {db_path}");
    print_org(&org);
    Ok(())
}

pub async fn cmd_org(db_path: &str, command: OrgCommand) -> Result<()> {
    let db = Db::open(db_path)
        .await
        .with_context(|| format!("opening {db_path}"))?;
    match command {
        OrgCommand::Create { name } => {
            let org = crate::org::create(&db.conn, &name).await?;
            print_org(&org);
        }
        OrgCommand::Rename { name } => {
            let org = crate::org::rename(&db.conn, &name).await?;
            print_org(&org);
        }
        OrgCommand::Delete { yes } => {
            let Some(org) = crate::org::get(&db.conn).await? else {
            ...
            crate::org::delete(&db.conn).await?;
        ...
        OrgCommand::Invite { email, role } => {
            ...
            let member = crate::members::invite(&db.conn, &email, role, None).await?;
            print_member(&member);
        }
        OrgCommand::Members { command } => match command {
            MembersCommand::List => {
                for member in crate::members::list(&db.conn).await? {
                    print_member(&member);
                }
            }
            MembersCommand::Remove { member_id } => {
                crate::members::remove(&db.conn, &member_id).await?;
        ...
        OrgCommand::Role { command } => match command {
            RoleCommand::Set { member_id, role } => {
                let role: Role = role
                    .parse()
                    .map_err(|e: crate::roles::InvalidRole| anyhow::anyhow!(e))?;
                let member = crate::members::set_role(&db.conn, &member_id, role).await?;
                print_member(&member);
            }
        },
    }
...
pub async fn cmd_keys(db_path: &str, command: KeysCommand) -> Result<()> {
    let db = Db::open(db_path).await?;
    match command {
        KeysCommand::Create { member, name } => {
            let (key, full_key) =
                crate::api_keys::create(&db.conn, &member, name.as_deref()).await?;
        ...
        KeysCommand::Revoke { key_id } => {
            crate::api_keys::revoke(&db.conn, &key_id).await?;
            println!("Revoked API key {key_id}");
        }
        KeysCommand::List => {
            for key in crate::api_keys::list(&db.conn).await? {
...
pub async fn cmd_status(db_path: &str) -> Result<()> {
    let db = Db::open(db_path).await?;
    let org = crate::org::get(&db.conn).await?;
    let members = crate::members::list(&db.conn).await?;
...
pub async fn cmd_serve(db_path: &str, listen: Option<String>) -> Result<()> {
    let db = Db::open(db_path).await?;
    if crate::org::get(&db.conn).await?.is_none() {
        bail!("no org exists yet — run `wardn init` first");
    }
    let listen_addr = listen.unwrap_or_else(config::default_listen_addr);

    #[cfg_attr(not(feature = "observability"), allow(unused_mut))]
    let mut state = serve::AppState::new(db.conn, crate::db::now());
