...
- **`GET /metrics` is unauthenticated**, registered outside `auth_middleware` alongside the existing `/healthz`. Standard Prometheus convention — scrapers typically can't do app-level auth, and unlike usage events (which carry `owner_id`), metrics expose only aggregate counts/histograms, no per-request identifying data beyond the `namespace` label (see below). Network-level access control (firewall/VPC) is the expected boundary, same as most Prometheus deployments and consistent with this project's existing loopback-only dev posture for `postgres`/`sqld` in `podman-compose.yml`.
...
- No `/metrics` authentication — accepted risk, same tier as this project's other loopback-only dev-service exposures.
