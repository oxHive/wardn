...
- **`GET /metrics` requires a static bearer token (`METRICS_TOKEN`)**, checked before rendering, superseding this slice's original brainstorming choice of "unauthenticated, Prometheus convention." Reversed during Task 2's review: at the time, the `namespace` label (see below) made an unauthenticated `/metrics` a tenant-enumeration endpoint — any caller could list every tenant UUID, request volume, and latency profile, and anonymous `POST /users` could grow that per-tenant label set without bound. A shared-secret token was a small addition (one new `Config` field, one header check) and closed the enumeration-read path. It did **not** address the growth-vector concern on its own — an unauthenticated caller could still repeatedly `POST /users` to grow the recorder's distinct-`namespace` label set, since the token only gated *reading* `/metrics`, not writing metrics. That growth vector was later closed outright by the security-hardening audit, which removed the `namespace` label from proxy metrics entirely rather than capping it (see below) — the token's remaining job is gating the traffic-volume/pool/queue-depth exposure described in `metrics_handler`'s doc comment (`src/observability.rs`). Distinct from the API-key system — this is a single deployment-wide secret, not a per-caller credential.
...
- ~~Namespace-label cardinality capping~~ — moot: the security-hardening audit removed the `namespace` label from proxy metrics entirely rather than capping it (see Scope decisions above and the metric catalog amendment).
