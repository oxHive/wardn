# hivemind-gateway Walking Skeleton Implementation Plan
...
Expected: FAIL to compile — `hivemind_gateway::app` doesn't exist yet, and this is a binary-only crate today with no library target for the test to link against.
...
```toml
[package]
name = "hivemind-gateway"
version = "0.1.0"
edition = "2024"

[lib]
name = "hivemind_gateway"
path = "src/lib.rs"
...
(Splitting into a `[lib]` + `[[bin]]` is what lets `tests/*.rs` integration tests import `hivemind_gateway::app()` — this is the standard axum project shape.)
...
Expected: FAIL to compile — `hivemind_gateway::db` doesn't exist yet.
...
Expected: FAIL to compile — `hivemind_gateway::auth` doesn't exist yet.
...
Expected: FAIL to compile — `hivemind_gateway::routing` doesn't exist yet.
...
Expected: FAIL to compile — `hivemind_gateway::app` currently takes no arguments (Task 1's signature), and `proxy.rs` doesn't exist.
...
```rust
use hivemind_gateway::{app, config::Config, db, AppState};
...
Update `tests/health_test.rs`'s `healthz_returns_ok` test to build a real `AppState` (same `test_pool()` helper pattern as the other test files) and call `hivemind_gateway::app(state)` instead of the old no-arg `app()`.
