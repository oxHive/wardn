# hivemind-gateway Observability Implementation Plan
...
Append to `tests/observability_test.rs` (add `use hivemind_gateway::auth::AppState;` etc. are already imported; add these new imports at the top of the file alongside the existing ones):
...
Add `observability` to the `use hivemind_gateway::{...}` import list at the top of `main.rs`.
...
```
Expected: at least one `"health":"up"` (the `hivemind-gateway` job). If it prints `"health":"down"` instead, check `podman-compose logs gateway` and `podman-compose logs prometheus` before proceeding — a down target here means Task 1-4's `/metrics` route or this task's scrape config has a mismatch, not something to paper over.
